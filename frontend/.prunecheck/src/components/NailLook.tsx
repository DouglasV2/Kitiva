...
import { HandPreview } from './beauty/HandPreview';
import { handPropsFromSpec } from './beauty/handFromSpec';
    ...
    <section className="nk" id="nail-look">
      ...
      <div className="nk-shell">
            ...
            <div className="nk-card" ref={specRef}>
              ...
              <div className="nk-cols">
                ...
                <div className="nk-preview">
                  ...
                  {/* Drawn in the browser from the same spec the kit is built from, so the picture and
                      the shopping list can never disagree. Replaces a server-rendered SVG string that
                      arrived over the wire only to be injected verbatim. */}
                  <div className="nk-stage" data-testid="nail-diagram">
                    <HandPreview {...handPropsFromSpec(d)} idPrefix="spec-" className="nk-hand" />
                  </div>
        ...
        {salon && (
          <SalonResult
            salon={salon}
            design={result?.brief?.design ?? brief?.design ?? null}
 ...
 */
function SalonResult({ salon, design, assumptions, prompt, onEditSpec }: {
  salon: SalonBrief;
  design: NailDesignSpec | null;
  ...
  async function downloadPng() {
    ...
    try {
      ...
      // Serialised from the node that is actually on screen, which is the only way the file can keep
      // the promise the comment above makes. Reading it back from the DOM also means the download needs
      // no second copy of the drawing code to drift out of step with the first.
      ...
      <div className="nk-brief-card nk-share" ref={shareRef}>
        <div className="nk-share-cols">
          <div className="nk-share-visual" data-testid="salon-diagram">
            <HandPreview {...handPropsFromSpec(design)} ref={handRef} idPrefix="salon-"
              className="nk-hand" />
          </div>
          <div>
